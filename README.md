[![Tests](https://github.com//ckanext-sk-demo/workflows/tests.yml/badge.svg)](https://github.com//ckanext-sk-demo/actions/workflows/test.yml)
...
function defined inside `ckanext.sk_demo.logic.action` will be
...
the module. As result, only this group will be available as `ckan sk_demo` CLI command.
...
* config options can be accessed by shorter name: `option()` instead of
  `tk.config["ckanext.sk_demo.option.name"]`.
...
`assets/webassets.yml` and include this named asset into template using `{% asset "sk_demo/ASSET_NAME" %}` tag.
...
to generate migrations for the model using `ckan generate migration -p sk_demo -m "Migration message"` CLI command.
...
`ckanext/sk_demo/tests` contains examples of tests for standard
...
`ckanext/sk_demo/tests/benchmarks` contains benchmarks. They
