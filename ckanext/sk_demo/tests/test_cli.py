"""Tests for ckanext.sk_demo.cli module.
...
import ckanext.sk_demo.cli as commands
...
class TestCommand:
    ...
    def test_output(self, cli: CliRunner, faker: Faker):
        ...
        # second argument of `cli.invoke`. For example, `sk_demo
        # a b -C 1 --xxx=2` becomes `cli.invoke(sk_demo, ["a",
        # "b", "-C", "1", "--xxx=2"])`
        result = cli.invoke(commands.sk_demo, ["command"])
        # `result.exit_code` contains `0` if command succeeded
        assert not result.exit_code
        # `result.output` contains normal output. `result.stderr` contains
        # output of an error
        assert result.output == "Hello, sk_demo\n"

        # use fake data instead of hardcoded test values to discover
        # output-based problems
        name = faker.word()
        result = cli.invoke(commands.sk_demo, ["command", name])
        assert result.output == f"Hello, {name}\n"

    def test_verbosity(self, cli: CliRunner):
        """Verbosity level appends exclamation marks."""
        result = cli.invoke(commands.sk_demo, ["command", "-v"])
        assert result.output == "Hello, sk_demo!\n"

        result = cli.invoke(commands.sk_demo, ["command", "-vvv"])
        assert result.output == "Hello, sk_demo!!!\n"
...
class TestCountUsers:
    ...
    def test_output(self, cli: CliRunner, faker: Faker, user_factory: Any):
        ...
        result = cli.invoke(commands.sk_demo, ["count-users"])
